...
agent.addListener('next', function (error, agent) {
  var window = jsdom.jsdom(agent.body.replace('</head>', '</head><body>')).createWindow();
  jsdom.jQueryify(window, 'jquery/jquery-1.4.2.js', function (window, jquery) {
    ...
    if (phones.length) {
      ...
      body.find('#thumbs img').each(function(){
        var id = jquery(this).attr('src').split(/\//).pop();
        phone.images.push({
          id: id,
          small: 'http://www.google.com/phone/image/small/' + id,
          medium: 'http://www.google.com/phone/image/medium/' + id
        });
      });
      fs.writeSync(fs.openSync(baseDir + phone.id + '.json', 'w'), JSON.stringify(phone));
    } else {
      var age = 0;
      body.find('li.list').each(function(a){
